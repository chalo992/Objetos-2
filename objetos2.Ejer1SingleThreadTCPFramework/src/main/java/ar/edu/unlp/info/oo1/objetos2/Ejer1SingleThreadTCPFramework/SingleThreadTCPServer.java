...
public abstract class SingleThreadTCPServer {
    ...
    private final void handleClient(Socket clientSocket) {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null) {
                System.out.println(this.receivedMessage(inputLine)
                        + this.clientSocketInfo(clientSocket));
                
                if (inputLine.equalsIgnoreCase(this.termination())) {
                    break; // Client requested to close the connection
                }
                handleMessage(inputLine, out);
            }
            System.out.println(this.connectionClosed() + this.clientSocketInfo(clientSocket));
        } catch (IOException e) {
    ...
    protected String clientSocketInfo(Socket clientSocket) {
    	return clientSocket.getInetAddress().getHostAddress() + ":"
                + clientSocket.getPort();
    }
    
    protected String receivedMessage(String inputLine) {
    	return "Received message: " + inputLine + " from ";
    }
    
    protected String connectionClosed() {
    	return "Connection closed with ";
    }
